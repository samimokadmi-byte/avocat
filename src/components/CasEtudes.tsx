...
export default function CasEtudes() {
  return (
    <section id="cas" className="bg-paper">
      <div className="px-8 py-16 md:py-section max-w-content mx-auto">

        {/* ── Section header ─────────────────────────────────────────── */}
        <div className="flex items-center gap-3 mb-10 md:mb-16">
          <span className="font-mono text-[11px] uppercase tracking-[0.08em] text-text2">[06]</span>
          <span className="w-8 h-px bg-hairline-strong flex-none" />
          <span className="font-mono text-[11px] uppercase tracking-[0.08em] text-text2">Études de Cas</span>
        </div>
        ...
        <div className="flex flex-col gap-6">
          {cas.map(({ secteur, problematique, solution, impact }) => (
            <div key={secteur} className="bg-paper-2 border border-hairline-strong p-6 md:p-10 grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10">
              <div className="flex flex-col justify-between gap-6">
                <div>
                  <p className="font-mono text-[11px] uppercase tracking-[0.08em] text-text2 mb-3">{secteur}</p>
              ...
              <div>
                <p className="font-mono text-[11px] uppercase tracking-[0.08em] text-text2 mb-3">Problématique</p>
                <p className="text-body text-text2 leading-relaxed">{problematique}</p>
              </div>
              <div>
                <p className="font-mono text-[11px] uppercase tracking-[0.08em] text-text2 mb-3">Solution apportée</p>
                <p className="text-body text-text2 leading-relaxed">{solution}</p>
              </div>
            </div>
          ))}
        </div>
