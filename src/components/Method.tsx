...
export default function Method() {
  return (
    <section id="methode" className="bg-ink">
      <div className="px-8 py-16 md:py-section max-w-content mx-auto">

        {/* ── Section header ─────────────────────────────────────────── */}
        <div className="flex items-center gap-3 mb-10 md:mb-16">
          <span className="font-mono text-[11px] uppercase tracking-[0.08em] text-paper/35">[03]</span>
          <span className="w-8 h-px bg-paper/15 flex-none" />
          <span className="font-mono text-[11px] uppercase tracking-[0.08em] text-paper/35">Méthode</span>
        </div>

        <h2 className="font-display text-heading text-paper font-normal mb-12 md:mb-16 max-w-xl text-pretty">
          Trois actes.{' '}
        ...
        <div className="flex flex-col">
          {steps.map((step, i) => (
            <div key={step.number} className="relative flex gap-8 md:gap-16 pb-12 md:pb-16 last:pb-0">
              {i < steps.length - 1 && (
                <div className="absolute left-[1.35rem] top-12 bottom-0 w-px bg-paper/12" />
              )}

              <div className="relative flex-none w-11 h-11 border border-paper/15 bg-ink-soft flex items-center justify-center">
              ...
              <div className="flex-1 pt-2">
                <div className="flex flex-col sm:flex-row sm:items-baseline sm:gap-4 mb-4">
                  <h3 className="font-display text-xl font-normal text-paper">{step.title}</h3>
                  <span className="inline-flex items-center border border-paper/12 rounded-full px-2.5 py-0.5 font-mono text-[11px] uppercase tracking-[0.06em] text-paper/35 mt-1.5 sm:mt-0 self-start">
                    {step.duration}
                  </span>
                </div>
